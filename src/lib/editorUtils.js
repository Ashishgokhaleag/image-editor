...
// Utility functions for editor transformations
  // Apply transformations to image or video element
  export const applyTransformation = (
    ...
    // Apply finetune adjustments
    if (options.finetune) {
      if (options.finetune.brightness !== 100) {
        filters.push(`brightness(${options.finetune.brightness / 100})`);
      }
      if (options.finetune.contrast !== 100) {
        filters.push(`contrast(${options.finetune.contrast / 100})`);
      }
      if (options.finetune.saturation !== 100) {
        filters.push(`saturate(${options.finetune.saturation / 100})`);
      }
      // Exposure approximated with brightness
      if (options.finetune.exposure !== 100) {
        filters.push(`brightness(${options.finetune.exposure / 100})`);
      }
    }
